...
def training(config, args):
    ...
    print('model loaded...')
    dataset = datasets.NSDTSEADataset(config, model)
    # dataset = get_dataset(config, model)
    print('Dataset loaded...')

    dataset_size = len(dataset)
    train_size = int(dataset_size * 0.9)
    validation_size = int(dataset_size * 0.1) + 1
    ...
    train_loader = DataLoader(train_dataset, batch_size = config['training']['batch_size'], sampler = train_sampler, num_workers = 4)
    valid_loader = DataLoader(valid_dataset, batch_size = config['training']['batch_size'], sampler = valid_sampler, num_workers = 4)
...
def inference(config, args):
    ...
    json_path = config['test']['json_path']
    filenames = sorted(make_dataset(json_path, float("inf")))
    ...
    for filename in filenames[:300]:
        ...
        if clean_input.size < 70000:
            continue
        inputs = {'noisy': noisy_input, 'clean': clean_input}

        output_filename_prefix = filename[-30:-4] + '_'
